...
public class TelaAdminController implements Initializable {
    ...
    public void deslogar(ActionEvent event) throws IOException {
        String titulo, cabecalho, mensagem;
        titulo = "Deslogar";
        cabecalho = "";
        mensagem = "Deseja mesmo deslogar seu usuário?";
        if(Alertas.alertaConfirmar(titulo,mensagem,cabecalho)){
    ...
    public void goCadastroDisciplina(ActionEvent event) throws IOException {
        FXMLLoader fxmloader = new FXMLLoader(getClass().getClassLoader().getResource("view/TelaCadastroDisciplina.fxml"));
        Parent root = fxmloader.load();
        Scene novaCena = new Scene(root);
        //Da acesso ao controller do ExibirAluno;
        TelaCadastroDisciplinaController controller = fxmloader.getController();
        user.setNome(nomeAdmin.getText());
        user.setCpf(matriculaAdmin.getText());
        controller.initData(user);
        //Pega a informação do Stage
